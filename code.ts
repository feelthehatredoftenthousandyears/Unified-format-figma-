...
figma.ui.on("message", (msg) => {
  if (msg.type === "RESULT") {
    ...
    const shapes: CodegenResult[] = [];

    const msg_c = msg.result.replace(/(-?\d+(\.\d+)?)px/g, "vwcalc($1)");
    console.log(msg);

    msg.cbArray.forEach((x: { [key: string]: any }) => {
      shapes.push({
        language: "PLAINTEXT",
        // code: msg.scssText,
        code: x['@include'],
    ...
    console.log(shapes);
    resolveByMessageIndex[msg.messageIdx]?.(shapes);
  ...
  return new Promise(async (resolve) => {
    ...
    if(node.type === "TEXT") {
      ...
      interface ScssObject {
        ...
        'line-height.value'?: string|number;
      ...
      console.log('777');
      const cbArray: object[] = [];
      uniqueArray.forEach(x => {
        let scssA: ScssObject = {};
        // console.log(x);
